"""
test_parse.py
Tests for word_import_service.parse_text() and word_import_service.parse_docx_bytes().
Run: python app/tests/test_parse.py  (no DB, no Eel required)
"""
...
from services.word_import_service import parse_text, parse_docx_bytes, check_duplicates
...
def create_sample_docx_bytes() -> bytes:
    """Helper to create an in-memory .docx file with paragraph blocks and a table."""
    doc = docx.Document()
    doc.add_heading("Gram Panchayat Property Tax List", level=1)
    
    # Add paragraph resident block
    doc.add_paragraph("Name: Ramesh Patil\nProperty ID: GP/2024/001\nWard: Ward 1\nPhone: 9876543210\nAmount: 2500\nAddress: Plot 12, Main Road")
    doc.add_paragraph("")
    
    # Add table resident records
    table = doc.add_table(rows=1, cols=6)
    hdr_cells = table.rows[0].cells
    headers = ["Property ID", "Name", "Ward", "Phone", "Amount", "Address"]
    for i, h in enumerate(headers):
        hdr_cells[i].text = h
        
    data_rows = [
        ("GP/2024/002", "Sunita Deshmukh", "Ward 2", "9123456789", "1800", "House 45, Shivaji Nagar"),
        ("GP/2024/005", "Anil Kulkarni", "Ward 2", "9654321098", "4500", "Plot 88, Market Area"),
    ]
...
def run():
    ...
    # Real .docx binary parsing test
    print(f"\n{sep}")
    print("  Case 6 — Real .docx Document Parsing (Paragraphs + Table)")
    docx_bytes = create_sample_docx_bytes()
    docx_parsed = parse_docx_bytes(docx_bytes)
    print(f"  Docx records extracted: {len(docx_parsed)}  (expected 3: GP/2024/001, GP/2024/002, GP/2024/005)")
    for rec in docx_parsed:
        clean = {k: v for k, v in rec.items() if not k.startswith("_")}
        print(f"    → {json.dumps(clean, ensure_ascii=False)}")
    assert len(docx_parsed) == 3, f"Expected 3 records from .docx, got {len(docx_parsed)}"
    print("  Status: ✓ PASS")
