...
def execute_write_txn(fn):
    """
    Run an arbitrary write function against a raw SQLite connection in a single
    atomic transaction protected by the global writer lock.
    """
    with _write_lock:
        conn = _writer_conn()
        try:
