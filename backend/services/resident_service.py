"""
backend/services/resident_service.py
...
def update_resident(resident_id: int, data: dict) -> dict | None:
    ...
    execute_write(
        """UPDATE residents
           ...
           WHERE id=?""",
        (
            data.get("name", existing["name"]).strip(),
            data.get("property_id", existing["property_id"]).strip(),
            data.get("ward", existing["ward"]).strip(),
            data.get("phone", existing["phone"]).strip(),
